...
## Plugins and system config

- **Context `plugins` block** — Declares which model providers, executors, and auxiliary plugins (loggers, memory, …) the Context may use; versions are pinned per `PluginReference`.
- **System config** — Optional `ecp.config.yaml` (or `--config`) can allow-list which **plugin IDs** may be enabled (`plugins.allowEnable`, `plugins.defaultEnable`, `plugins.security`) and configure **loggers** (`loggers`), tool servers, and model policy.

See [SETUP.md](https://github.com/executioncontrolprotocol/executioncontrolprotocol/blob/main/SETUP.md) for `allowEnable`, `defaultEnable`, and security options in the example config.
