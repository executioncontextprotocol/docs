...
- **System config:** Optional `ecp.config.yaml` (e.g. from `config/ecp.config.example.yaml`) for allow-listed **plugins**, **loggers**, and security. The CLI loads it from `--config`, `./ecp.config.yaml`, or `~/.ecp/config.yaml`.
