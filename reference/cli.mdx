...
Use it to allow-list **plugins** (`plugins.allowEnable`, `plugins.defaultEnable`, `plugins.security`), configure **loggers** (`loggers`), tool servers, and model defaults. Manage with `ecp config plugins`, `ecp config loggers`, etc. (`ecp config --help`).
