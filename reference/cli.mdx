...
The ECP CLI runs and validates Context manifests. Prefer installing the CLI once (via `npm link`) so you can use the `ecp` command from your shell.
...
Use it to allow-list extensions (`allowEnable`, `defaultEnable`) and set security options. See [SETUP.md](https://github.com/executioncontrolprotocol/executioncontrolprotocol/blob/main/SETUP.md) and `config/ecp.config.example.yaml`.
